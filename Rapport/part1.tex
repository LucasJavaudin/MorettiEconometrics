	...
	\fbox{\begin{minipage}{1\textwidth}
			...
			Movie theatres only get a prior, and estimate the appeal for the film from it. They are profit-maximizing firms, so they are incentivized to choose the number of screens for a given film following the prior.\\
	\end{minipage}}\\
	 \\
	\\
	...
	\fbox{\begin{minipage}{1\textwidth}
	Moretti suposes that consumers don't measure the utility $U_{i,j}$ he could get by watching the film, but recieves instead a noisy signal
	\end{minipage}}\\
	\begin{equation*}
	...
	\fbox{\begin{minipage}{1\textwidth}
			The exprected utiliy in the first week (i.e. before the film is being released) is estimated by the weighted average of the prior and the signal:
	\end{minipage}}\\
	\begin{center}
	...
	\\
	We note that that in the first week a positive difference $\alpha_{j}^{*}-X_{j}'\beta>0$ increases the probability of watching and conversely. The presence of this term is justified by the fact that\\
	...
	- the fact that his acquaintancies who saw the film had a sufficiently high expected (ex-ante) utility to do so, i.e. $\omega_{j} X_{j}'\beta+(1-\omega_{j})s_{p j}>q_{p 1}$, and that the $N_{i}-n_{i}$ other peers did not verify such inequality.\\
